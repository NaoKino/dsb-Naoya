...
# Extract the top 10 rows from the payments table
query <- "SELECT value, category, member_id FROM payments ORDER BY value DESC LIMIT 10"
results <- dbGetQuery(sky_westminster, query)

# Extract the top 10 rows from the aggregated payments data with member details using dplyr
results <- tbl(sky_westminster, "payments") %>%
  group_by(member_id) %>%
  summarise(total_value = sum(value)) %>%
  arrange(desc(total_value)) %>%
  slice_max(order_by = total_value, n = 10) %>%
  inner_join(tbl(sky_westminster, "members"), by = c("member_id" = "id")) %>%
...
-   Write the data frame to a csv file called `contributions-all.csv` in the `data` folder. Answer: Please find the code which satisfy instruction. However, some issues with writting the data frame to csv file, so I muted last part of code which is shown as below. #write.csv(contributions_all, file = "data/contributions-all.csv")
...
-   Write the data frame to a csv file called `all_consulting_jobs.csv` in the `data` folder. Answer: Please find the code which satisfy instruction. However, some issues with writting the data frame to csv file, so I muted last part of code which is shown as below. #write.csv(all_consulting_jobs, file = "data/all_consulting_jobs.csv", row.names = FALSE)
